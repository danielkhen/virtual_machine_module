...
| <a name="input_disks"></a> [disks](#input\_disks) | (Optional) A list of managed disk to attach to the virtual machines. | <pre>list(object({<br>    name                   = string<br>    storage_account_type   = string<br>    create_option          = string<br>    caching                = string<br>    disk_size_gb           = number<br>    disk_encryption_set_id = optional(string)<br>    disk_iops_read_write   = optional(number)<br>    disk_mbps_read_write   = optional(number)<br>    disk_iops_read_only    = optional(number)<br>    disk_mbps_read_only    = optional(number)<br>    upload_size_bytes      = optional(number)<br>  }))</pre> | `[]` | no |
